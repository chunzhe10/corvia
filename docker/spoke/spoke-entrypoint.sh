    ...
    curl -sf -X POST "${CORVIA_MCP_URL:-}" \
        ...
        -d "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/call\",
             \"params\":{\"name\":\"corvia_write\",\"arguments\":{
               \"scope_id\":\"corvia\",\"agent_id\":\"${CORVIA_AGENT_ID:-spoke-unknown}\",
               \"content_role\":\"finding\",\"source_origin\":\"workspace\",
               \"content\":\"Spoke ${CORVIA_AGENT_ID:-unknown} failed: ${msg}\"
             }}}" 2>/dev/null || true
}
...
clone_with_retry() {
    ...
    default_branch=$(gh api "repos/${REPO_URL##*/}" --jq '.default_branch' 2>/dev/null || echo "master")

    for i in 1 2 3; do
        if git clone --depth 100 --branch "${default_branch}" "${REPO_URL}" /workspace 2>&1; then
            return 0
        fi
        echo "Clone failed (attempt $i/3), retrying in 5s..." >&2
        sleep 5
    done
    return 1
}
...
# Write .mcp.json pointing to hub
...
echo "${MCP_CONFIG}" > /workspace/.mcp.json
echo "MCP config written: ${CORVIA_MCP_URL}"
