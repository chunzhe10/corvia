...
use crate::traits::{FullTextSearchable, TextSearchResult};
...
impl TantivyIndex {
    ...
    pub fn open(cache_dir: &Path, db: Arc<Database>) -> Result<Self> {
...
mod tests {
    ...
    fn create_test_db(dir: &Path) -> Arc<Database> {
    ...
    async fn test_tantivy_index_entry_and_search() {
        ...
        assert_eq!(results[0].entry_id, entry.id);
    }
    ...
    async fn test_tantivy_scope_isolation() {
        ...
        idx.index_entry(&entry_a).await.unwrap();
        idx.index_entry(&entry_b).await.unwrap();
        idx.flush().await.unwrap();

        let results_a = idx.search_text("tantivy", "scope-a", 10).await.unwrap();
        assert_eq!(results_a.len(), 1);
        assert_eq!(results_a[0].entry_id, entry_a.id);

        let results_b = idx.search_text("tantivy", "scope-b", 10).await.unwrap();
        assert_eq!(results_b.len(), 1);
        assert_eq!(results_b[0].entry_id, entry_b.id);
    }
